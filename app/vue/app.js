...
const GET = (url) => axios.get('/odata/v4/'+url)
const POST = (cmd,data) => axios.post('/browse'+cmd,data)

const environments = Vue.createApp ({
    ...
    methods: {
        ...
        // search: ({target:{value:v}}) => environments.fetch(v && '&$search='+v),

        async fetch (etc='') {
            const {data} = await GET(`environments/Environments`)
            environments.list = data.value
        },
...
//books.getUserInfo()
environments.fetch() // initially fill list of books
